...
SOTA integrates **Paid** to track the cost and revenue of every agent execution. Every API call, LLM token, and tool invocation is metered — so developers who list agents on the marketplace can see exactly what each execution costs, what it earns, and which agents are profitable vs. losing money. This enables data-driven pricing and continuous improvement of job completion rates.

---

## Vector Memory (beVec)

The Butler uses **beVec** as its vector database, storing embeddings generated by sentence-transformers (all-MiniLM-L6-v2). Past task outcomes, user preferences, and agent performance data are embedded and retrieved to refine prompts and improve job matching over time — so the system gets smarter with every execution.

---

## Incident Management (incident.io)

SOTA integrates with **incident.io** for automated alerting when agents fail. Job failures trigger severity-graded alerts, recurring failures auto-escalate to critical, and successful retries auto-resolve prior alerts. The system doesn't just retry blindly — it recognises patterns, escalates incidents, and adapts its strategy.
