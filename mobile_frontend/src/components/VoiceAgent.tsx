...
interface VoiceAgentProps {
  agentId?: string;
  butlerUrl?: string;
  onButlerMessage?: (message: any) => void;
  sidebarOpen?: boolean;
  orbVisible?: boolean;
  ctaVisible?: boolean;
  chatReady?: boolean;
...
const STABLECOIN_ADDRESS = (process.env.NEXT_PUBLIC_USDC_ADDRESS ||
  '0x0000000000000000000000000000000000000000') as `0x${string}`; // USDC on Base Sepolia
...
// Client tools that bridge ElevenLabs to Butler API
const createButlerTools = (butlerUrl: string, onMessage?: (msg: any) => void) => ({
  // Send user query to OpenAI-backed Butler Agent
  query_butler: async ({ query }: { query: string }) => {
    console.log('📤 Sending to Butler:', query);

    try {
      const response = await fetch(`${butlerUrl}/chat`, {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(`Butler error: ${response.statusText}`);
      }
  ...
  check_wallet_status: async ({ address }: { address: string }) => {
    ...
    try {
      const response = await fetch(`${butlerUrl}/wallet/${address}`);
      const wallet = await response.json();
      return `Balance: ${wallet.balance}. Active jobs: ${wallet.activeJobs}`;
    } catch (error) {
...
export const VoiceAgent: React.FC<VoiceAgentProps> = ({ 
  agentId,
  butlerUrl = 'http://localhost:3001/api/v1',
  onButlerMessage,
  sidebarOpen = false,
  orbVisible = true,
  ctaVisible = true,
  ...
  const conversation = useConversation({
    clientTools: createButlerTools(butlerUrl, onButlerMessage),

    onConnect: ({ conversationId }) => {
      console.log('✅ Connected to ElevenLabs:', conversationId);
      console.log('🔗 Connecting to Butler at:', butlerUrl);
      ...
      try {
        ...
        if (chainId && chainId !== TARGET_CHAIN_ID) {
          if (switchChainAsync) {
            ...
            console.log('✅ Switched chain to Base Sepolia');
          } else {
            setBidStatus('Switch to Base Sepolia to send bid');
