...
class TestPersistOutcome:
    ...
    @pytest.fixture
    def memory_postgres_only(self, mock_db):
        """TaskPatternMemory with PostgreSQL only (no Qdrant)."""
    ...
    async def test_success_outcome(self, memory_postgres_only, mock_db):
        job = FakeJob(tags=["hackathon_registration"])
        result = {"success": True, "hackathons": []}

        outcome = await memory_postgres_only.persist_outcome(
    ...
    async def test_captcha_failure(self, memory_postgres_only):
    ...
    async def test_network_failure(self, memory_postgres_only):
    ...
    async def test_unknown_failure(self, memory_postgres_only):
    ...
    async def test_postgres_called_with_dict(self, memory_postgres_only, mock_db):
        job = FakeJob(tags=["call_verification"])
        await memory_postgres_only.persist_outcome(
    ...
    async def test_no_qdrant_still_works(self, memory_postgres_only, mock_db):
        """Without Qdrant configured, persist should still write to PostgreSQL."""
        await memory_postgres_only.persist_outcome(
    ...
    async def test_no_incident_io_no_crash(self, memory_postgres_only):
    ...
    async def test_strategy_passed_through(self, memory_postgres_only):
        outcome = await memory_postgres_only.persist_outcome(
            job=FakeJob(), agent_id="hackathon",
    ...
    async def test_task_type_inferred(self, memory_postgres_only):
    ...
    async def test_postgres_error_swallowed(self):
        """PostgreSQL failure should not crash persist_outcome."""
        db = AsyncMock()
        db.store_task_outcome = AsyncMock(side_effect=RuntimeError("PostgreSQL down"))
