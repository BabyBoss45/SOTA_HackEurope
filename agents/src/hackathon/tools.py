...
Uses Anthropic Claude API and httpx for scraping.
...
class SearchHackathonsTool(BaseTool):
    ...
    async def execute(
        ...
        """Search for hackathons using Anthropic Claude + scraper fallback."""
        ...
        if api_key:
            prompt = (
                f"List upcoming hackathons and coding competitions "
                f"near {location} between {date_from} and {date_to}{kw_clause}.\n\n"
                f"Consider these sources:\n"
                f"1. lu.ma — Luma events\n"
                f"2. devpost.com — Devpost hackathons\n"
                f"3. mlh.io — Major League Hacking seasons\n"
                f"4. eventbrite.com — Eventbrite tech events\n"
                f"5. ethglobal.com — ETHGlobal events\n\n"
                f"CRITICAL — URL REQUIREMENTS:\n"
                f"- The 'url' field MUST be the direct link to the event page.\n"
            ...
            try:
                ...
                resp = await client.messages.create(
                    ...
                    system="You are a hackathon research assistant. Return ONLY valid JSON arrays. Never include past events.",
                    messages=[
                ...
                logger.info("Claude search returned %d hackathon(s)", len(hackathons))

            except Exception as e:
                logger.error("Claude hackathon search failed: %s", e)
...
class ScrapeHackathonDetailsTool(BaseTool):
    ...
    async def execute(self, url: str) -> str:
        """Scrape hackathon details from a URL using httpx + Anthropic Claude summarisation."""
        from anthropic import AsyncAnthropic
        ...
        try:
            ...
            raw = summary.content[0].text if summary.content else "{}"
