...
class EmbeddingModel:
    """Embedding provider with sentence-transformers (if available) and a deterministic fallback."""

    def __init__(self, model: str = "all-MiniLM-L6-v2") -> None:
        self.model = model
        self.st_model = self._try_sentence_transformers()
    ...
    def embed(self, text: str) -> List[float]:
        ...
        if self.st_model:
            try:
                ...
                logger.debug("SentenceTransformer embedding failed, falling back: %s", exc)
...
def _demo() -> None:
    ...
    print("- SentenceTransformer embeddings:", bool(filler.embedder.st_model))
