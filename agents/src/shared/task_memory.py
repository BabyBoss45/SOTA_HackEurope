...
Usage::
    ...
    memory = TaskPatternMemory(db=postgres_db)
...
class TaskPatternMemory:
    ...
    async def persist_outcome(
        ...
        # 1. PostgreSQL (always)
        await self._persist_postgres(outcome)
    ...
    async def _persist_postgres(self, outcome: TaskOutcome) -> None:
        if not self.db:
            return
        try:
            await self.db.store_task_outcome(asdict(outcome))
        except Exception as exc:
            logger.warning("PostgreSQL task outcome write failed: %s", exc)
