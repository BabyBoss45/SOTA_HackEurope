...
class EventType(str, Enum):
    """Contract event types we care about (OrderBook)"""
    JOB_POSTED = "JobCreated"        # OrderBook emits JobCreated
    BID_PLACED = "BidPlaced"
    BID_ACCEPTED = "BidAccepted"
    DELIVERY_SUBMITTED = "JobCompleted"  # OrderBook emits JobCompleted
...
class JobPostedEvent:
    """Parsed JobCreated event from OrderBook"""
    job_id: int
    client: str        # poster
    job_type: int      # not in event; default 0
    budget: int        # maxBudgetUsdc
    budget_usdc: int   # maxBudgetUsdc
...
class BidPlacedEvent:
    """Parsed BidPlaced event from OrderBook"""
    job_id: int
    bid_id: int
    bidder: str        # agent address
    amount: int        # priceUsdc
    amount_usdc: int   # priceUsdc (same)
...
class BidAcceptedEvent:
    """Parsed BidAccepted event from OrderBook"""
    job_id: int
    bid_id: int
    worker: str        # provider
...
class DeliverySubmittedEvent:
    """Parsed JobCompleted event from OrderBook"""
    job_id: int
...
class EventListener:
    ...
    async def _process_job_posted(self, event: dict):
        ...
        parsed = JobPostedEvent(
            ...
            budget=args.get('maxBudgetUsdc', 0),
            budget_usdc=args.get('maxBudgetUsdc', 0),
        ...
        logger.info("JobCreated evt job_id=%s poster=%s budget_usdc=%s tx=%s",
                    parsed.job_id, parsed.client, parsed.budget, parsed.tx_hash)
    ...
    async def _process_bid_placed(self, event: dict):
        ...
        parsed = BidPlacedEvent(
            ...
            amount=args.get('priceUsdc', 0),
            amount_usdc=args.get('priceUsdc', 0),
