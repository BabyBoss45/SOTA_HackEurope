...
class Database:
    ...
    async def store_task_outcome(self, data: dict) -> dict:
        """Persist a structured TaskOutcome record."""
        doc_id = await _next_id(self._adb, "taskOutcomes")
        record = {
            "id": doc_id,
            **data,
            "createdAt": data.get("created_at") or _now(),
        }
        await self._adb.collection("taskOutcomes").document(str(doc_id)).set(record)
        return record
    ...
    async def get_task_outcomes(
        ...
        """Query task outcomes, optionally filtered by agent and/or task type."""
        q = self._adb.collection("taskOutcomes")
        if agent_id:
