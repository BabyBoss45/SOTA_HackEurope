...
class BaseArchiveAgent(ABC):
    ...
    async def _execute_job_task(self, job: ActiveJob):
        ...
        if self.task_memory:
            try:
                pattern = await self.task_memory.analyze_similar(
                    description=job.description,
                    tags=list(getattr(job, "tags", [])),
        ...
        try:
            ...
            if self.task_memory:
                try:
                    await self.task_memory.persist_outcome(
                        job=job, agent_id=self.agent_type,
    ...
    async def _send_to_elevenlabs(self, event: BidAcceptedEvent, job_details: Any, job_metadata_uri: str = ""):
        ...
        Pulls metadata from job_metadata_uri (HTTP) to populate dynamic variables.
        """
