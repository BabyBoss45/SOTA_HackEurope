...
class BuildBookingContextTool(BaseTool):
    ...
    Requires BEVEC_ENDPOINT and ANTHROPIC_API_KEY to be configured.
    """
    parameters: dict = {
        "type": "object",
        "properties": {
...
class PostJobTool(BaseTool):
    ...
    name: str = "post_job"
    description: str = """
    Post a job to the OrderBook smart contract. Budget should be in USDC raw units
    (6 decimals). If you only have a float budget_usdc, provide that instead
    ...
    parameters: dict = {
        ...
        "properties": {
            ...
            "budget": {"type": "integer", "description": "Budget in raw USDC units (6 decimals)", "default": 0},
    ...
    async def execute(
        ...
        resolved_budget = budget or int(budget_usdc * 10**6)
        ...
        metadata_payload = {
            ...
            "budget_usdc": resolved_budget / 10**6 if resolved_budget else 0,
            "deadline": deadline,
            "tags": normalized_tags,
        }

        metadata_uri = f"ipfs://sota-metadata-{hashlib.sha256(json.dumps(metadata_payload, sort_keys=True).encode()).hexdigest()[:16]}"
        ...
        try:
            ...
            for bid in bids:
                ...
                formatted_bids.append({
                    "bid_id": bid_id,
                    "bidder": bidder,
                    "amount_usdc": amount / 10**6,  # Convert from raw USDC
