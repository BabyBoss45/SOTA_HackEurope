...
class ManagerAgent:
    ...
    async def post_booking_job(
        ...
        metadata_payload = {
            ...
            "budget_micro": budget,
            "budget_usdc": budget / 10**6 if budget else 0,
            "deadline": deadline,
            "tags": normalized_tags,
        }

        import json as _json
        metadata_uri = f"ipfs://sota-metadata-{hashlib.sha256(_json.dumps(metadata_payload, sort_keys=True).encode()).hexdigest()[:16]}"
